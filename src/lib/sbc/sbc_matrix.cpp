                      ...
                      )
{
  ...
  if (in->glob_pivot_index < 0)
    return;
      ...
      if (inout->glob_pivot_index >= 0)
        {
          if (fabs (in->norm_of_rev - inout->norm_of_rev) < EPS)
            {
              if (inout->glob_pivot_index < in->glob_pivot_index)
                {
                  inout->glob_pivot_index = in->glob_pivot_index;
                  inout->norm_of_rev = in->norm_of_rev;
                }
            }
          else if (in->norm_of_rev < inout->norm_of_rev)
              {
                inout->glob_pivot_index = in->glob_pivot_index;
                inout->norm_of_rev = in->norm_of_rev;
              }
      ...
      if (has_s_col)
        {
          ...
          if (matrix_reverse (block, coef_matrix, permutation, s))
            {
              fprintf (stderr, "rank = %d, in abort0\n", rank);
  ...
  if (rank == source)
    {
      ...
      if (matrix_reverse (block, coef_matrix, permutation, m))
        {
          fprintf (stderr, "rank = %d, in abort\n", rank);
          MPI_Abort (comm, NO_SOLUTION);
        }
