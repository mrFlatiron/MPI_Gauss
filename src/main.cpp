...
// = gilbert;
 = abs_i_minus_j;
  ...
  int col_count = (s != 0) ? l + 1 : l;
  int color; 
  if (p > col_count)
    {
      color = (rank > col_count - 1) ? 1 : 0;
...
*/
  ...
  if (n < 20)
  SBC_MPI_print (&loc_storage, glob_matrix, buf_print, comm);
...
//now x = the solution
  ...
  if (argc < 4)
    ...
    SBC_MPI_fill_in (&loc_storage, comm, glob_matrix, argv[3]);

  SBC_MPI_multi_vector (&loc_storage, x, b_new, comm, buf_); // b_new = Ax
